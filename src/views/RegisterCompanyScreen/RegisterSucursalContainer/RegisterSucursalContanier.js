...
const RegisterSucursalContainer = (props) => {
  ...
  const getBranchOffices = async () => {
    ...
    try {
      const res = await axios.post(
        `${BRANCHOFFICEHOUSE_URL}/branchofficebycompany`,
        dataSend
      );
      //console.log(res);
  ...
  const eliminar = () => {
    ...
    //http://localhost:3000/api/branchofficehouse
    axios
      .post(`${BRANCHOFFICEHOUSE_URL}/delete`, dataDelete)
      .then((res) => {
        //getBranchOffices();
        setData(
          data.filter(
            (elemento) =>
              elemento.branch_office_id !== selectsuc.branch_office_id
          )
        );
      })
                ...
                data.map((elemento) => (
                  <tr key={elemento.branch_office_id}>
                    ...
                    <td className="text-center">
                      {elemento.branch_office_name}
                    </td>
                    <td className="text-center">
                      {elemento.branch_office_cuit}
                    </td>
                    <td className="text-center">
                      <Button
                        color="primary"
